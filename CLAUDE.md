...
- **Compound fields are parsed together, not per-column.** `ParseShotsOnGoal`/`ParsePowerPlayShotsOnGoal`/`ParsePuckBattles` match a single regex against strings like `"Shots / on goal 5/3 60%"` and return a 3-tuple; there's no way to independently re-parse just the percentage.
- **Puck battles/recoveries/losses are per-game diagram totals, not season-average pairs.** Unlike most stats, `Puck recoveries`/`Puck losses`/`Puck battles` on page 2 only carry a single game-value column (no "season average" second column), and the report also renders a per-zone (offensive/neutral/defensive) breakdown beneath each one that isn't parsed — the zone labels are polluted by a team-name watermark in the text extraction, making them unreliable to regex against.
...
- **`Write` also generates a fully self-contained `Dashboard.html`** in the same directory, right after `output.csv` — it reads the embedded `Dashboard.html` resource as text (via reflection, the same `Assembly.GetManifestResourceStream` pattern `MainWindowViewModel.CopyResourceFile` uses), re-reads the just-written CSV, and substitutes an inline `<script id="dashboardDataScript">` (containing the JSON-encoded CSV text plus a `generatedAt` timestamp, assigned to `window.HUDL_READER_DASHBOARD_DATA`) in place of the `<!--HUDL_READER_DASHBOARD_DATA-->` placeholder comment in the template. This exists so the dashboard can auto-load its data with zero user action, even when opened directly from disk — a `fetch()` of a sibling `output.csv` is blocked by the browser under `file://`, but an inline `<script>` isn't. Both `Cli` and `UI` get a fully working, pre-loaded `Dashboard.html` for free since they share this method — this is a new capability for the CLI, which previously had no dashboard-generation step of its own at all.
...
- `StartProcessing` copies the embedded `Dashboard.html` resource into the output folder (via `Assembly.GetManifestResourceStream`, matched by filename suffix) *before* kicking off parsing on a background `Task.Run`, purely so the user has something to open immediately. That copy has no data embedded (the `<!--HUDL_READER_DASHBOARD_DATA-->` placeholder is untouched) and gets **overwritten** once parsing finishes — `InStatParser` → `CsvExportService.Write` regenerates `Dashboard.html` in the same spot with the CSV data baked in. The two writes never race since the second happens strictly after `await parser.ParsePlayerReports()` inside the same task.
...
- **Auto-load on page open prefers embedded data over `fetch('output.csv')`.** On `DOMContentLoaded` it first checks `window.HUDL_READER_DASHBOARD_DATA`, which is only defined if `CsvExportService` generated this specific copy of the file (see above) — that works even under `file://`, since it's embedded inline rather than fetched. The `fetch('output.csv')` path only runs as a fallback, for the case where this page is served over an actual web server without ever having been regenerated with embedded data (e.g. an old copy, or the raw un-processed template).
...
- **The header shows a "Data as of ..." timestamp** next to the player info, sourced differently depending on how the CSV was loaded: the embedded data's `generatedAt` (auto-load), the uploaded `File` object's `lastModified` (manual upload), or the `Last-Modified` response header (server-fetch fallback). `formatTimestamp()` accepts any of these shapes (ISO string, HTTP date string, or ms-epoch number).
- `exportDashboard()` strips the upload section, theme toggle, and `#dashboardDataScript` tag from the cloned static export (the last one specifically to avoid the exported file re-running `renderDashboard()` against canvases that no longer exist, since canvases are baked to `<img>` before export).
